...
class Team(Base):
    ...
    max_members = Column(
        Integer,
        default=6,
        comment="总席位（以 OpenAI 订阅的 seats_entitled 为准；仅在读取失败时才用默认值兜底）",
    )
    # 席位明细：按 OpenAI 的席型分开维护
    # default = Standard（普通席位），prolite = Premium（高级席位）
    seats_default_total = Column(Integer, default=0, comment="普通席位(default/Standard)已购总数")
    seats_default_assigned = Column(Integer, default=0, comment="普通席位(default/Standard)已分配数")
    seats_default_available = Column(Integer, default=0, comment="普通席位(default/Standard)可用数")
    seats_prolite_total = Column(Integer, default=0, comment="高级席位(prolite/Premium)已购总数")
    seats_prolite_assigned = Column(Integer, default=0, comment="高级席位(prolite/Premium)已分配数")
    seats_prolite_available = Column(Integer, default=0, comment="高级席位(prolite/Premium)可用数")
    seats_synced_at = Column(DateTime, comment="席位信息最后成功同步时间；为空表示从未读到真实席位")
