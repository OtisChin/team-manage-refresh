...
class RedeemFlowService:
    ...
    async def select_team_auto(
        ...
        try:
            ...
            # 兑换码默认占用 default(Standard) 席位：排除该席型已用尽的 Team
            self.team_service._append_default_seat_condition(conditions)
                ...
                try:
                    ...
                    async with _team_lock_manager.acquire(team_id_final):
                        ...
                        invite_res = await self.chatgpt_service.send_invite(
                            ...
                            # 用户自助兑换固定占用普通席位（default/Standard）；
                            # 高级席位(prolite)单价高得多，只允许管理员在后台显式分配。
                            seat_type=ChatGPTService.SEAT_TYPE_DEFAULT,
                        )
                        ...
                        try:
                            ...
                            # 待接受邀请不占席位：邀请发出后把刚才预留的座位放回池子。
                            # 占座只用于在 API 调用期间串行化并发兑换，不代表席位已被占用。
