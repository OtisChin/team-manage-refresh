...
class TeamService:
    ...
    # "读不到真实席位"时的兜底总席位（正常路径下会用 OpenAI 订阅里的 seats_entitled）
    DEFAULT_TEAM_MAX_MEMBERS = 6
    MIN_TEAM_MAX_MEMBERS = 1
    MAX_TEAM_MAX_MEMBERS = 100
    ...
    @staticmethod
    def _append_default_seat_condition(conditions: List[Any]) -> None:
        """兑换码默认走 default(Standard) 席位池，排除该席型已用尽的 Team。

        尚未同步到真实席位（``seats_synced_at`` 为空）的 Team 不做限制，
        避免把"未知"当成"没有席位"，交由服务端兜底报错。
        """
        conditions.append(
            or_(
                Team.seats_synced_at.is_(None),
    ...
    async def _handle_api_error(self, result: Dict[str, Any], team: Team, db_session: AsyncSession) -> bool:
        ...
        # 2. 判定是否为“席位已满”错误
        # 1.5 席型无余量：服务端提示需要先加购席位。
        # 这不代表整个 Team 满了（其他席型可能仍有空位），所以只报错、不翻 Team 状态，
        # 否则一次"往 prolite 邀请但 prolite 为 0"会把整个 Team 标成 full。
        seat_purchase_keywords = ["additional seat needed", "additional seats needed"]
        if any(kw in error_msg for kw in seat_purchase_keywords):
            logger.warning(
                "Team %s 目标席型无可用席位，需要先加购: %s", team.id, error_msg
            )
            return True
        ...
        if any(kw in error_msg for kw in full_keywords):
            ...
            team.status = "full"
            # 只有在还没读到真实席位时，才允许靠"撞墙"反推容量。
            # 这个反推只会向下修正，一旦 /subscriptions 读到权威席位就会被覆盖。
            if getattr(team, "seats_synced_at", None) is None:
                ...
                # 已有权威席位数据：只标满员，不用撞墙结果覆盖 seats_entitled，
                # 否则一次偶发误判会把真实席位永久改小且无法自动恢复。
                logger.warning(
                    "Team %s 已有真实席位数据(seats_synced_at=%s, max_members=%s)，本次满员不改写席位上限",
                    team.id,
                    team.seats_synced_at,
                    team.max_members,
                )
    ...
    async def _get_joined_mapping_count(self, team_id: int, db_session: AsyncSession) -> int:
        """统计本地已加入（joined）的成员数，用作同步时的人数下限。

        待接受邀请（invited）**不计入**：OpenAI 在对方接受之前不会为其分配席位
        （``/subscriptions`` 的 ``assigned`` 也不含待接受邀请），这里保持同口径，
        否则"不占位"的口径会被人数下限重新抬回去。
        """
    ...
    async def _apply_member_count_floor(
        ...
        """将远端同步人数与本地已加入成员数合并，避免短暂同步延迟把人数回写变小。"""
        await db_session.flush()
        joined_mapping_count = await self._get_joined_mapping_count(team.id, db_session)
        observed_count = int(observed_member_count or 0)
        effective_members = max(observed_count, joined_mapping_count)
    ...
    @staticmethod
    def _apply_seat_snapshot(team: Team, snapshot: Optional[Dict[str, Any]]) -> bool:
        """把 OpenAI 订阅的席位快照写入 Team，返回是否成功应用了真实席位。

        - ``max_members`` 跟随 ``seats_entitled``（真实已购席位总数），不再依赖写死的默认值；
        - ``seats_default_*`` / ``seats_prolite_*`` 分别记录普通席型与高级席型的明细；
        - ``seats_synced_at`` 作为"是否已读到真实席位"的标记，供满员纠错逻辑判断。
        """
        if not snapshot or not snapshot.get("success"):
            return False

        seats_entitled = int(snapshot.get("seats_entitled") or 0)
        if seats_entitled <= 0:
            return False

        seat_types = snapshot.get("seat_types") or {}
        default_seat = seat_types.get(ChatGPTService.SEAT_TYPE_DEFAULT) or {}
        prolite_seat = seat_types.get(ChatGPTService.SEAT_TYPE_PROLITE) or {}

        team.seats_default_total = int(default_seat.get("total") or 0)
        team.seats_default_assigned = int(default_seat.get("assigned") or 0)
        team.seats_default_available = int(default_seat.get("available") or 0)
        team.seats_prolite_total = int(prolite_seat.get("total") or 0)
        team.seats_prolite_assigned = int(prolite_seat.get("assigned") or 0)
        team.seats_prolite_available = int(prolite_seat.get("available") or 0)
        team.seats_synced_at = get_now()
        team.max_members = seats_entitled
        return True

    @staticmethod
    def _serialize_seat_fields(team: Team) -> Dict[str, Any]:
        """把按席型拆分的席位列序列化给前端（default=普通 / prolite=高级）。"""
        synced_at = getattr(team, "seats_synced_at", None)
        return {
            "pending_members": int(getattr(team, "pending_members", 0) or 0),
            "seats_default_total": int(getattr(team, "seats_default_total", 0) or 0),
            "seats_default_assigned": int(getattr(team, "seats_default_assigned", 0) or 0),
            "seats_default_available": int(getattr(team, "seats_default_available", 0) or 0),
            "seats_prolite_total": int(getattr(team, "seats_prolite_total", 0) or 0),
            "seats_prolite_assigned": int(getattr(team, "seats_prolite_assigned", 0) or 0),
            "seats_prolite_available": int(getattr(team, "seats_prolite_available", 0) or 0),
            "seats_synced_at": synced_at.isoformat() if synced_at else None,
        }

    @staticmethod
    def _get_seat_type_capacity(team: Team, seat_type: str) -> tuple[Optional[int], Optional[int]]:
        """返回某个席型的 (已购总数, 可用数)。

        数据来自上一次 ``/subscriptions`` 同步。若从未同步过真实席位，返回
        ``(None, None)``，调用方应据此跳过校验，而不是把"未知"误判成"没有席位"。
        """
        if not getattr(team, "seats_synced_at", None):
            return None, None

        if ChatGPTService.normalize_seat_type(seat_type) == ChatGPTService.SEAT_TYPE_PROLITE:
            return (
                int(getattr(team, "seats_prolite_total", 0) or 0),
                int(getattr(team, "seats_prolite_available", 0) or 0),
            )
        return (
            int(getattr(team, "seats_default_total", 0) or 0),
            int(getattr(team, "seats_default_available", 0) or 0),
        )

    @staticmethod
    def _check_seat_type_availability(
        team: Team, seat_type: str
    ) -> Optional[Dict[str, Any]]:
        """校验席型是否可邀请；不可用时返回结构化错误，可用时返回 None。

        席位池是"先买后用"的：往没有购买或已用尽的席型发邀请，服务端只会回
        "additional seat needed" 而不真正下发邀请，所以这里提前拦掉。
        """
        seat_total, seat_available = TeamService._get_seat_type_capacity(team, seat_type)
        if seat_total is None:
            return None  # 尚未同步到真实席位，交给服务端报错兜底

        label = ChatGPTService.SEAT_TYPE_LABELS.get(seat_type, seat_type)
        if seat_total <= 0:
            return {
                "error_code": "seat_type_unavailable",
                "error": f"该 Team 没有已购买的{label}，请先在 OpenAI 后台加购席位",
            }
        if seat_available is not None and seat_available <= 0:
            return {
                "error_code": "seat_type_full",
                "error": f"{label}已无可用余量，请先加购席位或改选其他席型",
            }
        return None

    async def _get_fallback_max_members(self, db_session: AsyncSession) -> int:
        """读取"无法从 OpenAI 读到真实席位"时使用的兜底总席位。"""
        ...
        try:
            value = int(str(raw_value or self.DEFAULT_TEAM_MAX_MEMBERS).strip() or self.DEFAULT_TEAM_MAX_MEMBERS)
        except (TypeError, ValueError):
            logger.warning("默认总席位配置无效: %s, 已回退到 %s", raw_value, self.DEFAULT_TEAM_MAX_MEMBERS)
            value = self.DEFAULT_TEAM_MAX_MEMBERS

        if value < self.MIN_TEAM_MAX_MEMBERS or value > self.MAX_TEAM_MAX_MEMBERS:
            logger.warning("默认总席位超出允许范围: %s, 已回退到 %s", value, self.DEFAULT_TEAM_MAX_MEMBERS)
            value = self.DEFAULT_TEAM_MAX_MEMBERS
        return value
    ...
    async def import_team_single(
        ...
        try:
            ...
            for selected_account in accounts_to_import:
                ...
                # 成员口径与同步保持一致：current_members 只算已加入成员，
                # 待接受邀请单独记在 pending_members（OpenAI 在其被接受前不分配席位）。
                joined_member_emails = set()
                invited_member_emails = set()
                if members_result["success"]:
                    for member in members_result.get("members", []):
                        normalized_member_email = self._normalize_member_email(member.get("email"))
                        if normalized_member_email:
                            joined_member_emails.add(normalized_member_email)
                ...
                for invite in pending_invites:
                    normalized_invite_email = invite.get("email_address")
                    if normalized_invite_email:
                        invited_member_emails.add(normalized_invite_email)

                current_members = len(joined_member_emails)
                ...
                # 确定总席位：以 OpenAI 订阅里的真实席位（seats_entitled）为准，
                # 只有在读取失败时才回退到系统设置里的兜底值。
                fallback_max_members = await self._get_fallback_max_members(db_session)

                subscription_result = await self.chatgpt_service.get_subscription(
                    access_token,
                    selected_account["account_id"],
                    db_session,
                    identifier=email,
                ...
                seat_snapshot = {
                    **subscription_result,
                    "success": bool(subscription_result.get("success")),
                }
                seats_entitled = int(subscription_result.get("seats_entitled") or 0)
                if seats_entitled > 0:
                    max_members = seats_entitled
                else:
                    max_members = fallback_max_members
                    logger.warning(
                        "Team %s 读取真实席位失败，回退兜底总席位 %s: %s",
                        selected_account["account_id"],
                        fallback_max_members,
                        subscription_result.get("error"),
                    )
                ...
                # 写入按席型拆分的真实席位明细（default=普通 / prolite=高级）
                self._apply_seat_snapshot(team, seat_snapshot)

                # 立刻建立成员映射：自动踢人里的"非授权成员清退"和"后台邀请过期踢人"
                # 两条策略完全依赖 team_email_mappings，导入时若不建立映射，
                # 新导入的 Team 在第一次同步之前一个候选都扫不出来。
    ...
    async def sync_team_info(
        ...
        try:
            ...
            # current_members 只统计"已加入"成员：OpenAI 在对方接受邀请之前不会为其
            # 分配席位，所以待接受邀请不占位，只单独记在 pending_members 里。
            current_members = 0
            pending_members = 0
            if members_result["success"]:
            ...
            if invites_result["success"]:
                ...
                pending_members = len(pending_invites)
            ...
            # 7.6 刷新真实席位（按席型拆分）；失败时沿用上一次的席位数据，
            #     不把 max_members 改回兜底默认值。
            subscription_result = await self.chatgpt_service.get_subscription(
                access_token,
                current_account["account_id"],
                db_session,
            ...
            if not self._apply_seat_snapshot(team, seat_snapshot):
                logger.warning(
                    "Team %s 同步真实席位失败，沿用现有席位配置: %s",
                    team.id,
                    subscription_result.get("error"),
                )
    ...
    async def get_team_members(
        ...
        try:
            ...
            # 只把"已加入"计入成员数；待接受邀请单独统计，不占席位
            joined_count = sum(1 for m in all_members if m.get("status") == "joined")
            live_member_count = joined_count
            team.pending_members = sum(1 for m in all_members if m.get("status") != "joined")
            team.last_sync = get_now()
    ...
    async def add_team_member(
        ...
        try:
            ...
            # 席型校验：席位池先买后用，缺席位时服务端不会真正下发邀请
            normalized_seat_type = ChatGPTService.normalize_seat_type(seat_type)
            if normalized_seat_type is None:
                return self._admin_error(
                    "invalid_seat_type",
                    f"不支持的席位类型: {seat_type}",
                )

            seat_check = self._check_seat_type_availability(team, normalized_seat_type)
            if seat_check:
                return self._admin_error(
                    seat_check["error_code"],
                    seat_check["error"],
                )
            ...
            # 待接受邀请不占席位：只累加待接受计数，不动 current_members / status。
            # （OpenAI 在对方接受之前也不会为其分配席位，两侧口径保持一致。）
            if not had_active_mapping:
                team.pending_members = (team.pending_members or 0) + 1
            await db_session.commit()
    ...
    async def add_team_members(
        ...
        # 席型校验：没有购买或已用尽时直接失败，避免逐个邮箱白白同步一遍
        normalized_seat_type = ChatGPTService.normalize_seat_type(seat_type)
        if normalized_seat_type is None:
            return self._admin_error(
                "invalid_seat_type",
        ...
        _, seat_available = self._get_seat_type_capacity(team, normalized_seat_type)
        if seat_available is not None:
            available_slots = min(available_slots, int(seat_available))
    ...
    async def get_available_teams(
        ...
        try:
            ...
            self._append_default_seat_condition(conditions)
    ...
    async def get_team_by_id(
        ...
        try:
            ...
            team_data = {
                ...
                "current_members": team.current_members,
                "max_members": team.max_members,
                **self._serialize_seat_fields(team),
                "status": team.status,
                "account_role": team.account_role,
                "device_code_auth_enabled": team.device_code_auth_enabled,
