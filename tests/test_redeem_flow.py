...
class TeamServiceBulkInviteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_team_member_does_not_consume_seat_for_pending_invite(self):
        """待接受邀请不占席位：邀请成功后成员数不变，只累加待接受计数。"""
        ...
        async with self.session_factory() as session2:
            team = await session2.get(Team, 101)
            # 待接受邀请不占席位：成员数保持 5，不会因为发出邀请而立刻变成满员
            self.assertEqual(team.current_members, 5)
            self.assertEqual(team.pending_members, 1)
            self.assertEqual(team.status, "active")
...
class RedeemFlowServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_virtual_welfare_code_handles_concurrent_redemptions_up_to_capacity(self):
        async with self.session_factory() as session:
            ...
            async with self.session_factory() as verify_session:
                ...
                # 待接受邀请不占席位：邀请发出后预留座位会被释放，成员数回到 0
                self.assertEqual(stored_team.current_members, 0)
    ...
    async def test_apply_member_count_floor_ignores_pending_invites(self):
        """待接受邀请不占位：人数下限只由已加入成员决定。"""
        async with self.session_factory() as session:
            team = Team(
                id=68,
            ...
            team_service = TeamService()
            await team_service.upsert_team_email_mapping(68, "joined@example.com", "joined", session, source="sync")
            await team_service.upsert_team_email_mapping(68, "pending1@example.com", "invited", session, source="redeem")
            await team_service.upsert_team_email_mapping(68, "pending2@example.com", "invited", session, source="redeem")
            await session.commit()

            effective_members = await team_service._apply_member_count_floor(team, 1, session)
            await session.commit()

            # 远端观察到 1 人 + 本地 1 个 joined = 1；两个 invited 不参与下限
